...
actionCleanup = function()
    ...
    if mythic_action.animation and mythic_action.animation.propTask then
        local propTask = GetClosestObjectOfType(GetEntityCoords(ped), 30.0, GetHashKey(mythic_action.animation.propTask), false, false, false)
        ESX.Game.DeleteObject(propTask)
    end
    if isProp then
        --local propNet = NetToObj(prop_net)
        DetachEntity(prop_net, 1, 1)
        ESX.Game.DeleteObject(prop_net)
    end
    prop_net = nil
end

setPropNet = function(val)
    prop_net = val
end
    ...
    while true do
        if isDoingAction then
            ...
            if not isProp and mythic_action.prop ~= nil and mythic_action.prop.model ~= nil then
                ...
                ESX.Game.SpawnLocalObject(mythic_action.prop.model, vector3(pCoords.x, pCoords.y, pCoords.z), function(object) 
                    ...
                    AttachEntityToEntity(object, GetPlayerPed(PlayerId()), GetPedBoneIndex(GetPlayerPed(PlayerId()), mythic_action.prop.bone), mythic_action.prop.coords.x, mythic_action.prop.coords.y, mythic_action.prop.coords.z, mythic_action.prop.rotation.x, mythic_action.prop.rotation.y, mythic_action.prop.rotation.z, 1, 1, 0, 1, 0, 1)
                    setPropNet(object)
                end)
                isProp = true
            end
